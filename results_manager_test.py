...
class TestDatabase(unittest.TestCase):
    def setUp(self):
        ...
        if models_TEST.Result.select().count() < 1:
            ...
            {'home_team': 'brighton', 'away_team': 'arsenal', 'home_htg': 2,
                          'away_htg': 1, 'home_ftg': None, 'away_ftg': 1}]
...
class TestResultsValidator(unittest.TestCase):
    def test_goals_are_integers(self):
        ...
        message,outcome = rv.validate_goal_types({'FT_Home':"1",'HT_Home':1,'FT_Away':2,'HT_Away':1})
        self.assertFalse(outcome)
        message,outcome = rv.validate_goal_types({'FT_Home':"",'HT_Home':1,'FT_Away':2,'HT_Away':1})
        self.assertFalse(outcome)
        message,outcome = rv.validate_goal_types({'FT_Home':1,'HT_Home':1,'FT_Away':2,'HT_Away':1})
    ...
    def test_FTGoals_not_less_than_HTGoals_and_postive_number(self):
        ...
        self.assertFalse(outcome)
        message,outcome = rv.validate_goal_values({'FT_Home':0,'HT_Home':0,'FT_Away':4,'HT_Away':-2})
        self.assertFalse(outcome)
    ...
    def test_team_name_are_present(self):
        ...
        message,outcome = rv.validate_team_names_present({'Home':'AFC','Away':'BFC','FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertTrue(outcome)
        message,outcome = rv.validate_team_names_present({'Home':'','Away':'BFC','FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertFalse(outcome)
        message,outcome = rv.validate_team_names_present({'Home':'AFC','Away':'','FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertFalse(outcome)
        message,outcome = rv.validate_team_names_present({'Home':None,'Away':'','FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertFalse(outcome)
        message,outcome = rv.validate_team_names_present({'Home':'AFC','Away':None,'FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertFalse(outcome)
        message,outcome = rv.validate_team_names_present({'Home':None,'Away':None,'FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertFalse(outcome)
        message,outcome = rv.validate_team_names_present({'Home':'','Away':'','FT_Home':2,
                                          'HT_Home':1,'FT_Away':3,'HT_Away':3})
        self.assertFalse(outcome)
    ...
    def test_validate_result_not_duplicate(self):
        ...
        """
        message,outcome = rv.result_is_new({'Home':'manchester city','Away':'bournemouth'})
        self.assertFalse(outcome)
        message,outcome = rv.result_is_new({'Home':'burnley','Away':'bournemouth'})
        self.assertFalse(outcome)
        message,outcome = rv.result_is_new({'Home':'brighton','Away':'manchester city'})
        self.assertTrue(outcome)
